...
describe("Ejercicios de Objetos-Funciones", () => {
  ...
  it('espero encontrar una función "addProperty" que devuelva un objeto copiado con la nueva propiedad y valor si el boolean es true, o devuelva el mismo objeto si es false', () => {
    expect(index.addProperties).not.toBeUndefined();
    const jon = { name: "Jon", surname: "Snow" };
    const newJon = index.addProperties(jon, "colorHair", "black", true);
    expect(newJon).toStrictEqual({ ...jon, colorHair: "black" });
    const originalJon = index.addProperties(jon, "colorHair", "black", false);
    expect(originalJon).toStrictEqual(jon);
  });
